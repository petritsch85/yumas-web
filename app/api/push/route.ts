...
// Minimal GET so we can test the route is reachable
export async function GET() {
  return NextResponse.json({ ok: true, route: 'push' });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user_id) return NextResponse.json({ ok: true, reason: 'no user_id' });
    ...
    if (!subs?.length) return NextResponse.json({ ok: true, reason: 'no subscriptions' });
