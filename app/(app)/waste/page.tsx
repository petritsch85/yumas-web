  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('waste.title')}</h1>
        <button
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-5 mb-6 flex items-center gap-4">
        <div>
          ...
          <div className="text-sm text-gray-500">{t('waste.totalWasteMonth')}</div>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {REASON_OPTIONS.map((r) => (
          ...
          >
            {r === 'all' ? t('waste.allReasons') : t(`status.${r}`)}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100">
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-400 text-sm">{t('waste.noLogs')}</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('waste.table.date')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('waste.table.item')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('waste.table.location')}</th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">{t('waste.table.qty')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('waste.table.reason')}</th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">{t('waste.table.estCost')}</th>
                </tr>
              </thead>
              <tbody>
