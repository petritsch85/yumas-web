        ...
        {otherProfiles.length > 0 && (
          <div className="px-3 mb-4">
            ...
            {otherProfiles.map(p => {
              ...
              const hasUnread = unread.has(roomId);
              return (
                <button
                  key={p.id}
        ...
        <div className={`rounded-2xl px-3 py-2 text-sm ${
          ...
          {msg.media_url && msg.media_type === 'video' && (
            <video
              src={msg.media_url}
              controls
              className="rounded-xl max-h-64 max-w-full mb-1"
            />
          )}
          {msg.content && (
...
export default function ChatPage() {
  ...
  /* ── All active profiles (for DM list) ── */
  ...
  useEffect(() => {
    setUnread(prev => { const n = new Set(prev); n.delete(activeRoom); return n; });
    if (myId) {
  ...
  /* ── Realtime: unread badge watcher (all rooms) ── */
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (msg.room !== activeRoomRef.current) {
          setUnread(prev => new Set([...prev, msg.room]));
        }
      })
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  ...
  return (
    <div className="flex h-full bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
      ...
      {showMobileSidebar && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setShowMobileSidebar(false)}>
          <div className="absolute inset-0 bg-black/30" />
          <div className="absolute top-0 left-0 bottom-0 w-64 bg-white shadow-2xl z-50" onClick={e => e.stopPropagation()}>
            <RoomSidebar
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3 flex-shrink-0">
          <button
            ...
            className="md:hidden p-1 -ml-1 text-gray-400 hover:text-gray-600 transition-colors"
          >
            <ChevronLeft size={20} />
          </button>
