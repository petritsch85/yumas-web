...
export default function NewRecipePage() {
  ...
  const handleCreate = async () => {
    ...
    if (!primaryName) { setError(t('recipes.nameRequired')); return; }
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 pb-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('recipes.newRecipe')}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{t('recipes.newRecipeSubtitle')}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-50">
          <p className="text-sm font-semibold text-gray-900">{t('recipes.recipeName')}</p>
        </div>
        ...
        <div className="px-5 py-4">
          ...
          {nameTab === 'en' && (
            <p className="text-xs text-gray-400 mt-1.5">
              This becomes the primary name used as a fallback for other languages.
            </p>
          )}
        </div>
      </div>
      ...
      >
        <Save size={16} />
        {saving ? t('recipes.creating') : t('recipes.createRecipe')}
      </button>
    </div>
  );
}
