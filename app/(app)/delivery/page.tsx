...
import { Truck, Printer, RefreshCw, CheckCircle2, AlertCircle, Package } from 'lucide-react';
...
export default function DeliveryPage() {
  ...
  /* ─ Query: existing run for target date ─ */
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // 1. For each store: get latest inventory submission + delivery targets
      const storeData: {
        store: Store;
      ...
      for (const { store, submission, targets } of storeData) {
        for (const target of targets) {
          ...
          const deliveryQty = Math.max(0, targetQty - reportedQty);
      ...
      // 5. Update status: 'ready' if all stores have submissions, else 'draft'
  ...
  const storeHasSubmission = (store: Store) => {
    // If lines exist for this store, we had a submission at generation time
    const sl = storeLines(store);
    if (sl.length === 0) return run !== null ? true : false; // no targets → trivially ok
    // Proxy: if all reported_qty are 0, might have no submission — but we track it during generation
    // Better: just show warning only when run hasn't been generated yet or store has no lines at all
    return sl.length > 0;
  };
