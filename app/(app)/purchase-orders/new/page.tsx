...
export default function NewPOPage() {
  ...
  const handleSave = async (submitForApproval: boolean) => {
    if (!supplierId || !locationId) { setError(t('purchaseOrders.new.errorSelectSupplierLocation')); return; }
    if (cart.length === 0)          { setError(t('purchaseOrders.new.errorAddProduct')); return; }
    ...
    try {
      ...
      const lines = cart.map((l) => ({
        po_id: po.id, supplier_product_id: l.supplier_product_id,
        display_name: l.display_name, einheit: l.einheit, item_id: null,
        quantity_ordered: l.qty, quantity_received: 0,
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('purchaseOrders.new.title')}</h1>
      </div>

      <div className="space-y-5">
        {/* Order details */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
          <h2 className="font-semibold text-gray-900 mb-4">{t('purchaseOrders.new.orderDetails')}</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t('purchaseOrders.new.supplier')}</label>
              <select className={inputCls} value={supplierId} onChange={(e) => { setSupplierId(e.target.value); setCart([]); setSearch(''); }}>
                <option value="">{t('purchaseOrders.new.selectSupplier')}</option>
                {suppliers?.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t('purchaseOrders.new.destination')}</label>
              <select className={inputCls} value={locationId} onChange={(e) => setLocationId(e.target.value)}>
                <option value="">{t('purchaseOrders.new.selectLocation')}</option>
                {locations?.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t('purchaseOrders.new.expectedDelivery')}</label>
              <input type="date" className={inputCls} value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">{t('purchaseOrders.new.notes')}</label>
              <input className={inputCls} placeholder={t('common.optional') + '…'} value={notes} onChange={(e) => setNotes(e.target.value)} />
            </div>
          </div>
        </div>
        ...
        {supplierId && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
            <h2 className="font-semibold text-gray-900 mb-3">{t('purchaseOrders.new.productCatalog')}</h2>
            <div className="relative mb-3">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input
                className={`${inputCls} pl-8`}
                placeholder={t('purchaseOrders.new.searchProducts')}
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
            </div>
            {!catalog || catalog.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-6">{t('purchaseOrders.new.noProductsForSupplier')}</p>
            ) : (
                    ...
                    <div key={p.id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50">
                      ...
                      <button
                        ...
                        className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${
                          added ? 'bg-red-50 text-red-400 hover:bg-red-100' : 'bg-green-50 text-[#1B5E20] hover:bg-green-100'
                        }`}
                      >
        ...
        {cart.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
            <h2 className="font-semibold text-gray-900 mb-4">
              {t('purchaseOrders.new.orderLines')} — {cart.length} {cart.length !== 1 ? t('purchaseOrders.new.productPlural') : t('purchaseOrders.new.productSingular')}
            </h2>
            <div className="grid grid-cols-[1fr_80px_80px_80px_80px_32px] gap-2 mb-2 px-1">
              {[
                t('purchaseOrders.new.product'),
                t('purchaseOrders.new.unitCol'),
                t('purchaseOrders.new.qty'),
                t('purchaseOrders.new.unitPrice'),
                t('purchaseOrders.new.total'),
                '',
              ].map((h, i) => (
                <p key={i} className="text-xs font-semibold text-gray-400 uppercase">{h}</p>
              ))}
            </div>
            {cart.map((line) => (
            ...
            <div className="border-t border-gray-100 mt-3 pt-3 flex justify-end">
              <span className="text-sm font-bold text-gray-900">
                {t('purchaseOrders.new.orderTotal')} {total > 0 ? `€${total.toFixed(2)}` : '—'}
              </span>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 justify-end">
          ...
          >
            {t('purchaseOrders.new.saveAsDraft')}
          </button>
          <button
          ...
          >
            {saving ? t('purchaseOrders.new.saving') : t('purchaseOrders.new.submitForApproval')}
          </button>
        </div>
      </div>
    </div>
